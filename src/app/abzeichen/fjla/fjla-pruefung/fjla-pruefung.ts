...
@Component({
  selector: 'app-fjla-pruefung',
  imports: [
    FormsModule,
    NgClass
  ],
  templateUrl: './fjla-pruefung.html',
  styleUrl: './fjla-pruefung.css',
})
export class FjlaPruefung implements OnInit {
  // routing
  router = inject(Router);
  // Fragen
  fragen = signal<string[]>([]);
  // antworten für Falsch beantwortete Fragen
  richtig = signal<string[]>([]);
  ...
  befuellen(){
    ...
    let zahlen = <number[]>[];
    while(this.fragen().length != 10) {
      let zahl = this.getRandomBetween(0, 39)
      if(!zahlen.includes(zahl)) {
        this.fragen().push(fragen()[zahl]);
        this.richtig().push(richtig()[zahl]);
        zahlen.push(zahl);
      }
    }
  }
